...
class PoCMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[46,92,184,368], stem_channel=16, fc_dim=1280,
                 ...
                 depths=[2,2,10,2], qk_ratio=1, sr_ratios=[8,4,2,1], dp=0.1, need_return_dict = False, size_type = 0):
        super().__init__()
        self.type = size_type # "s":0 "m":1 "l":2 
        ...
        self.stem_maxpooling1 = nn.AvgPool2d(kernel_size=2, stride=2)
        #self.stem_maxpooling1 = nn.MaxPool2d(kernel_size=3, stride=1, padding = 1)
        ...
        self.stem_maxpooling2 = nn.AvgPool2d(kernel_size=2, stride=2)
        #self.stem_maxpooling2 = nn.MaxPool2d(kernel_size=3, stride=1, padding = 1)
        ...
        #self.stem_maxpooling3 = nn.MaxPool2d(kernel_size=2, stride=2)
        #self.stem_maxpooling3 = nn.MaxPool2d(kernel_size=3, stride=1, padding = 1)
        ...
        # Classifier head
        if(self.type == 0):
            fc_in_dim = embed_dims[0]
        elif(self.type == 1):
            fc_in_dim = embed_dims[1]
        else:
            fc_in_dim = embed_dims[-1]

        self._fc = nn.Conv2d(fc_in_dim, fc_dim, kernel_size=1)
...
class CMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[46,92,184,368], stem_channel=16, fc_dim=1280,
                 num_heads=[1,2,4,8], mlp_ratios=[3.6,3.6,3.6,3.6], qkv_bias=True, qk_scale=None, representation_size=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., hybrid_backbone=None, norm_layer=None,
                 depths=[2,2,10,2], qk_ratio=1, sr_ratios=[8,4,2,1], dp=0.1, need_return_dict = False):
        super().__init__()
        self.need_return_dict = need_return_dict
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dims[-1]
        ...
        self.stem_relu1 = nn.GELU()
        self.stem_norm1 = nn.BatchNorm2d(stem_channel, eps=1e-5)
        #self.stem_maxpooling1 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.stem_conv2 = nn.Conv2d(stem_channel, stem_channel, kernel_size=3, stride=1, padding=1, bias=True)
        self.stem_relu2 = nn.GELU()
        self.stem_norm2 = nn.BatchNorm2d(stem_channel, eps=1e-5)
        #self.stem_maxpooling2 = nn.MaxPool2d(kernel_size=2, stride=2)
...
def PoCMT_ti(img_size, in_chans=1, num_classes=2, need_return_dict = True, size_type = 2):
    """ 
    CMT-Tiny
    """
    model_kwargs = dict(
        img_size=img_size, in_chans=in_chans, num_classes=num_classes, need_return_dict = need_return_dict, size_type = size_type,
        qkv_bias=True)
    model = PoCMT(**model_kwargs)
    return model

def PoCMT_xs(img_size, in_chans=1, num_classes=2, need_return_dict = True, size_type = 2):
    model_kwargs = dict(
        img_size=img_size, in_chans=in_chans, num_classes=num_classes, need_return_dict = need_return_dict, size_type = size_type,
        qkv_bias=True, embed_dims=[52,104,208,416], stem_channel=16, num_heads=[1,2,4,8],
...
def PoCMT_b(img_size, in_chans=1, num_classes=2, need_return_dict = True, size_type = 2):
    "base line"
    model_kwargs = dict(
        img_size=img_size, in_chans=in_chans, num_classes=num_classes, need_return_dict = need_return_dict, size_type = size_type,
        qkv_bias=True, embed_dims=[76,152,304,608], stem_channel=38, num_heads=[1,2,4,8],
...
def PoCMT_s(img_size, in_chans=1, num_classes=2, need_return_dict = True, size_type = 2):
    model_kwargs = dict(
        img_size=img_size, in_chans=in_chans, num_classes=num_classes, need_return_dict = need_return_dict, size_type = size_type,
        qkv_bias=True, embed_dims=[64,128,256,512], stem_channel=32, num_heads=[1,2,4,8],
